...
public class DefaultGatewayRouter implements IGatewayRouter{
	...
	public void init() {
		...
		//连接端口事件处理
		config.getRoutingServer().onEvent(GGEvents.Connection.CLOSE, (Void data) -> {
		...
		//连接成功事件
		config.getRoutingServer().onEvent(GGEvents.Connection.OPEN, (Void data) -> {
			GGSession session = GGSessionUtil.getSession();
			MessageDispatcher dispatcher = dispatchers.get(session);
			if (dispatcher == null) {
				dispatcher = new MessageDispatcher(config, session);
